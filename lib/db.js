...
// ── CORRECTION RECORD (no balance update) ──────────────────────
export async function correctAccountBalance(telegramId, accountId, newBalance, note, operationId) {
  const results = await db.batch([
    {
      sql: `INSERT OR IGNORE INTO transactions
            (telegram_id, account_id, type, amount, note, category, source, is_balance_correction, operation_id, created_at)
            SELECT ?, id,
                   CASE WHEN ? >= balance THEN 'masuk' ELSE 'keluar' END,
                   ABS(? - balance), ?, '⚙️ Koreksi', '', 1, ?, datetime('now')
            FROM accounts WHERE id = ? AND telegram_id = ?`,
      args: [String(telegramId), newBalance, newBalance, note || "Koreksi saldo manual", operationId, accountId, String(telegramId)],
    },
    {
