...
export async function initDB() {
  ...
  // Phase 2: Add columns to existing tables in parallel (idempotent ALTER operations)
  await Promise.all([
...
export async function addCorrectionRecord(telegramId, accountId, type, amount, note) {
  await db.execute({
    sql: `INSERT INTO transactions (telegram_id, account_id, type, amount, note, category, source)
          VALUES (?, ?, ?, ?, ?, '⚙️ Koreksi', '')`,
    args: [String(telegramId), accountId, type, amount, note || "Koreksi saldo manual"],
  });
}
